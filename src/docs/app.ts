...
const components: ComponentDoc[] = [
  {
    ...
    variantsHtml: () => buttonVariants.map((v) => `<fx-button variant="${v}">${v}</fx-button>`).join(''),
    controls: [
      { kind: 'select', attr: 'variant', label: 'Variante', options: buttonVariants, value: 'primary' },
      { kind: 'select', attr: 'size', label: 'Tamanho', options: sizes, value: 'md' },
      { kind: 'select', attr: 'type', label: 'Type (formulário)', options: ['button', 'submit', 'reset'], value: 'button' },
    ...
    attributes: [
      { name: 'variant', type: `'${buttonVariants.join("' | '")}'`, default: `'primary'`, desc: 'Estilo visual do botão.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho (altura via token, fonte e padding).' },
      { name: 'type', type: `'button' | 'submit' | 'reset'`, default: `'button'`, desc: 'Tipo do button nativo (útil em forms).' },
      { name: 'full', type: 'boolean', default: 'false', desc: 'Ocupa 100% da largura do container.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita a interação e aplica estilo esmaecido.' },
      { name: 'loading', type: 'boolean', default: 'false', desc: 'Exibe spinner interno e impede cliques.' },
    ],
    events: [{ name: 'click', type: 'MouseEvent', desc: 'Evento nativo de clique (suprimido quando disabled ou loading).' }],
    slots: [
      { name: '(padrão)', desc: 'Texto/rótulo do botão.' },
      { name: 'icon', desc: `Ícone antes do rótulo (<span slot="icon">→</span>).` },
    ],
  },
  {
    tag: 'fx-badge',
    ...
    variantsHtml: () => badgeVariants.map((v) => `<fx-badge variant="${v}">${v}</fx-badge>`).join(''),
    controls: [
      { kind: 'select', attr: 'variant', label: 'Variante', options: badgeVariants, value: 'primary' },
      { kind: 'toggle', attr: 'round', label: 'Arredondado (round)' },
    ],
    attributes: [
      { name: 'variant', type: `'${badgeVariants.join("' | '")}'`, default: `'neutral'`, desc: 'Cor semântica do badge.' },
      { name: 'round', type: 'boolean', default: 'false', desc: 'Formato pílula/circular — ideal para contadores.' },
    ],
  ...
  },
  {
    ...
    variantsHtml: () => `<fx-select clearable searchable placeholder="Selecione um estado"><option value="sp">São Paulo</option><option value="rj">Rio de Janeiro</option><option value="mg">Minas Gerais</option></fx-select>`,
    controls: [
      { kind: 'select', attr: 'size', label: 'Tamanho', options: sizes, value: 'md' },
      { kind: 'text', attr: 'placeholder', label: 'Placeholder', hint: 'Texto quando vazio' },
      { kind: 'text', attr: 'value', label: 'Value', hint: 'ex.: rj' },
      { kind: 'toggle', attr: 'searchable', label: 'Busca (searchable)', on: true },
      { kind: 'toggle', attr: 'clearable', label: 'Limpar (clearable)', on: true },
      { kind: 'toggle', attr: 'disabled', label: 'Desabilitado' },
    ],
    attributes: [
      { name: 'value', type: 'string', default: "1ª opção ou `selected`", desc: 'Valor selecionado; reflete para o atributo ao escolher.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho do campo.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita o campo.' },
      { name: 'placeholder', type: 'string', default: "''", desc: 'Texto exibido quando nada está selecionado.' },
      { name: 'searchable', type: 'boolean', default: 'false', desc: 'Exibe campo de pesquisa dentro do dropdown.' },
      { name: 'search-placeholder', type: 'string', default: "'Pesquisar…'", desc: 'Placeholder do campo de pesquisa.' },
      { name: 'no-results', type: 'string', default: "'Nenhum resultado'", desc: 'Mensagem quando a busca não encontra nada.' },
      { name: 'clearable', type: 'boolean', default: 'false', desc: 'Exibe botão × para limpar a seleção (visível quando há valor).' },
    ],
    events: [{ name: 'change', type: `CustomEvent<{ value: string }>`, desc: 'Emitido ao selecionar (composed — atravessa o Shadow DOM).' }],
    slots: [{ name: '(padrão)', desc: 'Elementos <option> nativos, espelhados para dentro do componente.' }],
  },
  {
    tag: 'fx-input',
    title: 'Input',
    ...
    variantsHtml: () =>
      ['text', 'number', 'email', 'password', 'search']
        ...
        .join(''),
    controls: [
      { kind: 'select', attr: 'type', label: 'Tipo', options: ['text', 'number', 'email', 'password', 'search', 'tel', 'url'], value: 'text' },
      { kind: 'select', attr: 'size', label: 'Tamanho', options: sizes, value: 'md' },
      { kind: 'text', attr: 'placeholder', label: 'Placeholder', hint: 'Texto de apoio' },
    ...
    attributes: [
      { name: 'type', type: `'text' | 'number' | 'email' | 'password' | 'search' | 'tel' | 'url'`, default: `'text'`, desc: 'Tipo do campo nativo.' },
      { name: 'value', type: 'string', default: "''", desc: 'Valor do campo; reflete para o atributo ao digitar.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho do campo.' },
      { name: 'placeholder', type: 'string', default: "''", desc: 'Texto de apoio.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita o campo.' },
      { name: 'readonly', type: 'boolean', default: 'false', desc: 'Somente leitura.' },
      { name: 'min / max / step', type: 'string', default: '—', desc: 'Restrições para type="number".' },
    ],
    events: [
      { name: 'input', type: `CustomEvent<{ value: string }>`, desc: 'Emitido a cada tecla (composed).' },
      { name: 'change', type: `CustomEvent<{ value: string }>`, desc: 'Emitido ao confirmar o valor (composed).' },
    ],
  },
  {
    tag: 'fx-multiselect',
    ...
    controls: [
      { kind: 'select', attr: 'size', label: 'Tamanho', options: sizes, value: 'md' },
      { kind: 'text', attr: 'placeholder', label: 'Placeholder', hint: 'Texto quando vazio' },
      { kind: 'text', attr: 'values', label: 'Values (CSV)', hint: 'ex.: sp,rj' },
      { kind: 'text', attr: 'no-results', label: 'Msg. sem resultado', hint: 'ex.: Nada encontrado' },
      { kind: 'toggle', attr: 'searchable', label: 'Pesquisa', on: true },
      { kind: 'toggle', attr: 'clearable', label: 'Limpar', on: true },
      { kind: 'toggle', attr: 'disabled', label: 'Desabilitado' },
    ],
    attributes: [
      { name: 'values', type: 'string[] (CSV no atributo)', default: '[]', desc: 'Valores selecionados; reflete ao alterar. Propriedade `values` aceita array.' },
      { name: 'searchable', type: 'boolean', default: 'false', desc: 'Exibe campo de pesquisa no popover.' },
      { name: 'clearable', type: 'boolean', default: 'false', desc: 'Botão × para limpar toda a seleção.' },
      { name: 'placeholder', type: 'string', default: `'Selecione…'`, desc: 'Texto quando nada está selecionado.' },
      { name: 'no-results', type: 'string', default: `'Nenhum resultado'`, desc: 'Mensagem quando a pesquisa não encontra nada.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho do campo.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita o componente.' },
    ],
    events: [{ name: 'change', type: `CustomEvent<{ values: string[] }>`, desc: 'Emitido ao adicionar/remover/limpar (composed).' }],
    slots: [{ name: '(padrão)', desc: 'Elementos <option> nativos, espelhados para dentro do componente.' }],
  },
  {
    tag: 'fx-switch',
    ...
    attributes: [
      { name: 'checked', type: 'boolean', default: 'false', desc: 'Estado ligado; reflete para o atributo ao clicar.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita o interruptor.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho da trilha.' },
    ],
    events: [{ name: 'change', type: `CustomEvent<{ checked: boolean }>`, desc: 'Emitido ao alternar (composed).' }],
  ...
  },
  {
    ...
    controls: [
      { kind: 'select', attr: 'mode', label: 'Modo', options: ['single', 'range', 'multiple'], value: 'single' },
      ...
      { kind: 'text', attr: 'min', label: 'min', hint: '2026 | 2026-03 | 2026-03-15' },
      { kind: 'text', attr: 'max', label: 'max', hint: '2026-12' },
    ],
    attributes: [
      { name: 'mode', type: `'single' | 'range' | 'multiple'`, default: `'single'`, desc: 'Um dia, período (início/fim) ou múltiplas datas.' },
      { name: 'value', type: 'string (ISO)', default: "''", desc: 'Data selecionada no modo single, ex.: 2026-08-15.' },
      { name: 'start / end', type: 'string (ISO)', default: "''", desc: 'Período selecionado no modo range.' },
      { name: 'values', type: 'CSV de ISO', default: "''", desc: 'Múltiplas datas no modo multiple, ex.: values="2026-08-05,2026-08-12".' },
      { name: 'min / max', type: 'ISO parcial', default: "''", desc: 'Limites por ano ("2026"), mês ("2026-03") ou dia ("2026-03-15"). Datas fora do limite ficam desabilitadas.' },
      { name: 'week-starts-on', type: `0 | 1`, default: `0`, desc: 'Início da semana: 0 = domingo, 1 = segunda.' },
    ],
    events: [{ name: 'change', type: `CustomEvent`, desc: 'Composed. Detail conforme o modo: `{ value }` · `{ value, start, end }` · `{ values: string[] }`.' }],
    slots: [],
  },
  {
    tag: 'fx-datepicker',
    ...
    controls: [
      ...
      { kind: 'toggle', attr: 'show-time', label: 'Hora (hh:mm:ss)', on: true },
      { kind: 'toggle', attr: 'free-text', label: 'Digitar livre' },
      { kind: 'text', attr: 'format', label: 'format', hint: 'dd/mm/yyyy HH:MM:SS' },
      { kind: 'toggle', attr: 'disabled', label: 'Desabilitado' },
      { kind: 'select', attr: 'size', label: 'Tamanho', options: sizes, value: 'md' },
    ],
    attributes: [
      { name: 'mode', type: `'single' | 'range' | 'multiple'`, default: `'single'`, desc: 'Tipo de seleção. Em single, o popover fecha ao escolher.' },
      { name: 'value', type: 'string (ISO)', default: "''", desc: 'Data (single), ex.: 2026-08-15.' },
      { name: 'start / end', type: 'string (ISO)', default: "''", desc: 'Período (range).' },
      { name: 'values', type: 'CSV', default: "''", desc: 'Múltiplas datas, ex.: values="2026-08-01,2026-08-15".' },
      { name: 'min / max', type: 'ISO parcial', default: "''", desc: 'Repasse direto ao calendário: ano ("2026"), mês ("2026-03") ou dia.' },
      { name: 'format', type: 'string', default: "'' (ISO)", desc: 'Máscara de exibição. Tokens: `dd mm yyyy` (data) e `HH MM SS` (hora, minuto, segundo) — ex.: dd/mm/yyyy HH:MM:SS.' },
      { name: 'show-time', type: 'boolean', default: 'false', desc: 'Inclui seleção de hora/minuto/segundo no popover; o valor passa a ser YYYY-MM-DDTHH:mm:ss.' },
      { name: 'clearable', type: 'boolean', default: 'false', desc: 'Mostra × para limpar quando há valor.' },
      { name: 'free-text', type: 'boolean', default: 'false', desc: 'Permite digitar a data no input (Enter ou blur aplica). Valida formato, data real e min/max; inválido reverte e emite `invalid`.' },
      { name: 'placeholder', type: 'string', default: `'dd/mm/aaaa'`, desc: 'Texto do input vazio.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita o campo.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Altura do campo.' },
    ],
    events: [
      { name: 'change', type: `CustomEvent`, desc: 'Composed. Detail conforme o modo: `{ value }` · `{ value, start, end }` · `{ values }`; vazio ao limpar.' },
      { name: 'invalid', type: `CustomEvent<{ text }>`, desc: 'Emitido no modo free-text quando o texto digitado é rejeitado (formato/data real/min-max).' },
    ],
    slots: [],
  },
    {
    ...
    attributes: [
      { name: 'checked', type: 'boolean', default: 'false', desc: 'Marcado (é refletido no host ao clicar).' },
      { name: 'indeterminate', type: 'boolean', default: 'false', desc: 'Estado misto; clicar resolve para checked.' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita.' },
      { name: 'value', type: 'string', default: "''", desc: 'Valor associado (via detail de change).' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho do controle.' },
    ],
    events: [{ name: 'change', type: `CustomEvent<{ checked: boolean; value: string }>`, desc: 'Émisso composto ao alternar.' }],
    ...
    controls: [{ kind: 'select', attr: 'size', label: 'Tamanho', options: sizes, value: 'md' }],
    attributes: [{ name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Diâmetro do spinner.' }],
  },
  {
    tag: 'fx-radio',
    title: 'Radio',
    ...
    demoHtml: (a) => `<fx-radio ${a} name="gender">Masculino</fx-radio><fx-radio ${a} name="gender">Feminino</fx-radio>`,
    variantsHtml: () =>
      ['sm', 'md', 'lg']
    ...
    attributes: [
      { name: 'checked', type: 'boolean', default: 'false', desc: 'Marcado (reflete no host).' },
      { name: 'disabled', type: 'boolean', default: 'false', desc: 'Desabilita.' },
      { name: 'value', type: 'string', default: "''", desc: 'Valor associado (via detail de change).' },
      { name: 'name', type: 'string', default: "''", desc: 'Grupo: radios com o mesmo name alternam exclusivamente.' },
      { name: 'size', type: `'sm' | 'md' | 'lg'`, default: `'md'`, desc: 'Tamanho do controle.' },
    ],
    events: [{ name: 'change', type: `CustomEvent<{ checked: boolean; value: string }>`, desc: 'Emitido quando este radio passa a ser o do grupo.' }],
  ...
  },
  {
    ...
    attributes: [
      ...
      { name: 'sort-field', type: 'string', default: "''", desc: 'Ordenação inicial.' },
      { name: 'sort-order', type: `'asc' | 'desc' | ''`, default: "''", desc: 'Direção inicial.' },
    ],
    events: [
      { name: 'sort-change', type: `CustomEvent<{ field: string; direction: 'asc' | 'desc' }>`, desc: 'Ao ordenar.' },
      { name: 'filter-change', type: `CustomEvent<{ field: string; value: string }>`, desc: 'Ao filtrar uma coluna.' },
      { name: 'page-change', type: `CustomEvent<{ page: number; rows: number }>`, desc: 'Ao trocar de página.' },
      { name: 'row-click', type: `CustomEvent<{ row: any; index: number }>`, desc: 'Clique na linha.' },
    ],
    slots: [
      { name: 'header[slot]', desc: 'Slot de header customizado por coluna (via <fx-column>).' },
  ...
  },
  {
    ...
    controls: [
      { kind: 'select', attr: 'variant', label: 'Variante', options: ['on', 'in', 'over'] },
      { kind: 'toggle', attr: 'error', label: 'Erro' },
    ],
    attributes: [
      { name: 'variant', type: `'on' | 'in' | 'over'`, default: `'on'`, desc: 'Posição da label: "on" (borda, padrão), "in" (placeholder interno), "over" (acima do campo).' },
      { name: 'error / invalid', type: 'boolean', default: 'false', desc: 'Indica estado de erro (rótulo e borda em vermelho).' },
      { name: 'success / valid', type: 'boolean', default: 'false', desc: 'Indica estado de sucesso (rótulo e borda em verde).' },
      { name: 'active', type: 'boolean', default: 'false', desc: 'Refletido automaticamente quando o campo contém valor, foco ou dropdown aberto.' },
    ],
    events: [],
    slots: [
  ...
  },
  {
    ...
    variantsHtml: () => {
      const p = (id: string, txt: string) => `<fx-tab-panel tab="${id}"><p style="margin:8px 0">${txt}</p></fx-tab-panel>`;
      return [
    ...
    controls: [
      { kind: 'select', attr: 'value', label: 'Aba ativa (tab id)', options: ['perfil', 'seguranca', 'notificacoes'], value: 'perfil' },
    ],
    attributes: [{ name: 'value', type: 'string', default: "'primeira aba'", desc: 'Tab ativa.' }],
    events: [
      { name: 'change', type: `CustomEvent<{ value: string }>`, desc: 'Ao trocar de aba.' },
    ],
  },
  {
    tag: 'fx-progress',
