...
export class FxCalendar extends FxElement {
	...
	private emit(): void {
		if (this.mode === "range") {
			const detail = {
				start:
					this.pickStart !== null
						? this.iso(this.pickStart)
						: this.getAttr("start", ""),
				end:
					this.pickEnd !== null
						? this.iso(this.pickEnd)
						: this.getAttr("end", ""),
			};
	...
	protected override render(): void {
		...
		if (this.view === "days") {
			const wds = [...Array(7)].map(
				(_, i) => this.fmt({ weekday: "short" }, new Date(2024, 8, 1 + i)), // 01/09/2024 foi domingo
			);
			cells =
		...
		this.root
			...
			.forEach((b) =>
				b.addEventListener("click", () =>
					this.navigate(Number(b.dataset.nav) as -1 | 1),
				),
			);
	...
	private daysHtml(
		todayK: number,
		sel: ReturnType<FxCalendar["selection"]>,
		_loc: string,
	): string {
